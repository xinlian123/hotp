...
public class HOTPService {
    ...
    public void sendHOTP2() throws AuthLoginException {
        try {
            sentHOTPCode = otpGenerator.generateOTP(getSharedSecret(), getMovingFactor(), codeLength, false, 16);
        } catch (NoSuchAlgorithmException e) {
            DEBUG.error("HOTP.sendHOTPCode() : " + "no such algorithm", e);
        ...
        sendHOTP2(sentHOTPCode, messageSubject, messageContent);
        sentHOTPCodeTime = currentTimeMillis();
    }
    ...
    public static void twilio(String tCode, String phoneNum) {
	    
  	  try {
  	    String url = "https://internalwstst.creditacceptance.com/customerContactHub/sendmessage";
  		URL obj = new URL(url);
  		...
  		String urlParameters = "{\"callingSystem\":\"IDM\",\"messageTemplate\":\"IDM_OTP\",\"communicationChannel\":\"TEXT\",\"messageFields\":{\"oneTimePass\":\"" + tCode + "\"},\"sendTo\":{\"phoneNumber\":\"" + phoneNum + "\"}}";
    ...
    private void sendHOTP2(String otpCode, String subject, String message) throws AuthLoginException {
